...
def callback(indata, frames, callback_time, status):
    ...
    if not silent:
        ...
        if max_probability> probability_threshold and top_index == 1: #A Stutter Event has been detected
            detection_time = int(time()*1000)
            random_hospital = np.random.randint(0,len(sensor_list))
            print(f"Stuttering Detected  ------> at time {detection_time} in {sensor_list[random_hospital]}")
            output_dict = {
                "sensor": sensor_list[random_hospital],
                "timestamp" : detection_time,
                "stutteringEvent" : 1, 
                }
            output_string = json.dumps(output_dict)
            client.publish('s296721', output_string)
