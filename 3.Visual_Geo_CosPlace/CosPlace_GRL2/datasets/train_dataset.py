...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, args, dataset_folder, M=10, alpha=30, N=5, L=2,
        ...
        ###############################################################
        self.domain_folder = args.domain_data      
        
        self.all_domain_images = os.listdir(self.domain_folder)
        self.all_domain_images.sort()
        self.train_domain = self.all_domain_images[0:int(args.domain_test_train_spilt*len(self.all_domain_images))]       
        ###############################################################
    ...
    def __getitem__(self, class_num):
        ...
        ##########################################
        domian_id = class_num % len(self.train_domain)
        domain_path = self.domain_folder+'/' +self.train_domain[domian_id]
        try:
            domain_image = open_image(domain_path)
        except Exception as e:
            logging.info(f"ERROR domain image {domain_path} couldn't be opened, it might be corrupted.")
            raise e
        ...
        tensor_domain =  T.functional.to_tensor(domain_image)

        tensor_domain = T.functional.resize(tensor_domain, size=[512,512])
        ...
        label_d1 = torch.zeros([2])
        label_d1[0] = 1

        label_d2 = torch.zeros([2])
        label_d2[1] = 1
        
        return tensor_image, class_num, image_path,label_d1,tensor_domain,label_d2
